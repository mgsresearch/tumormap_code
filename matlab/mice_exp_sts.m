...
idx_mice_use                        = "sts_tumor_exp_1";

path_data_unique                     = "mice/sts_tumor_example/";
...
pts_xyz_for_fig_system = pts_obj_use_vis;
...
if is_exist_actual_file == 2
    traj_actual = load( file_label_actual ).traj_actual; 
else
   
    figure(3);clf;
    scatter( pts_obj_use(:,1), pts_obj_use(:,2), [], color_post_use, 'filled', "SizeData", 150 ); 
    grid on; 
    axis equal;
    title("label the actual cutting"); 
    h = drawfreehand; 
    traj_actual = h.Position; 
    
    figure(4);clf;
    scatter( pts_obj_use(:,1), pts_obj_use(:,2), [], color_post_use, 'filled', "LineWidth", 5 ); 
    hold on; 
    plot( traj_actual(:,1), traj_actual(:,2), "r*" ); 
    title("show the gt"); 
    grid on; 
    axis equal;

    % traj_gt = traj_gt;
    save( file_label_actual, "traj_actual" );
...
%% figure-1: OCT mapping

% oct surface scanning
figure(26);clf;
pcshow(pts_xyz_redefine - mean( pts_xyz_redefine, 1 ) , pts_rgb_redefine,  "MarkerSize", 800);
...
% path_save_fig = strcat( path_save_img_global, idx_mice_use, "_oct_pre_.jpg" ); 
% exportgraphics(gcf, path_save_fig );
...
% path_save_fig = strcat( path_save_img_global, idx_mice_use, "_oct_tumor_map_post_resection.jpg" ); 
% exportgraphics(gcf, path_save_fig );