...
% global base path (local) + python transfer file path
idx_exvivo_use                      = "fiber_cg_2"; 
path_data_unique                     = "exvivo/fiber/";
...
cg_avg_tumor = load( strcat(path_tissue_information, "cg_avg_tumor.mat") ).cg_avg_tumor; 
cg_avg_healthy = load( strcat(path_tissue_information, "cg_avg_healthy.mat") ).cg_avg_healthy; 
cg_avg_thres = load( strcat(path_tissue_information, "cg_avg_thres.mat") ).cg_avg_thres; 

% threshold bar
cg_range_min_and_max = abs(cg_avg_healthy - cg_avg_tumor); 
cg_upper_bound_thres = cg_avg_thres + cg_range_min_and_max * confidence_scale; 
cg_lower_bound_thres = cg_avg_thres - cg_range_min_and_max * confidence_scale; 
...
for idx_id = 1 : size(spectral_npy, 1)
    ...
    % single feature map 
    if feature_tmp > cg_upper_bound_thres
        
        % healthy = 1
        label_id_tmp = 1;
        
    elseif feature_tmp <= cg_upper_bound_thres
...
x_robot_grid_center_tumorid = -120; 
...
pc_target_cut_down = pcdownsample( pc_targert_cut, 'gridAverage', 0.50 );
...
% 
% path_save_fig = strcat( path_save_img_global, idx_exvivo_use, "_tumor_boundary.jpg" ); 
...
% path_save_fig = strcat( path_save_img_global, idx_exvivo_use, "_tumor_tag.jpg" ); 
% exportgraphics(gcf, path_save_fig );